...
def Test(dataset: Data, model, device, topK, flag_multicore, test_batch_size, long_tail=False, temperature = 0.05):
    model = model.eval()
    if flag_multicore == 1:
    ...
    # attack_model_results = {'recall': np.zeros(len(topK))}
    with torch.no_grad():
        ...
        for batch_users in mini_batch(users, batch_size=test_batch_size):
            ...
            batch_users_device = torch.Tensor(batch_users).long()
...
def Atack_Test(dataset: Data, model, device, topK, flag_multicore, test_batch_size, long_tail=False, temperature = 0.05):
    model = model.eval()
    ...
    with torch.no_grad():
        ...
        file_name = '/select_user_' + args.dataset + '_' + str(94) + '.pkl'
        with open(file_path + file_name, 'rb') as f:
            users = pkl.load(f)
        # users =  # get user list to test
        users_list, rating_list, ground_true_list = [], [], []
        num_batch = len(users) // test_batch_size + 1
        long_tail_rate = 0.
        for batch_users in mini_batch(users, batch_size=test_batch_size):
            ...
            rating = rating.cpu()
            del rating
            users_list.append(batch_users)#要评价的user，10个batch
            rating_list.append(rating_k.cpu())#得到去除positive的topk的物品
            ground_true_list.append(ground_true)#得到测试集的item
        assert num_batch == len(users_list)
        enum_list = zip(rating_list, ground_true_list)
        ...
        if long_tail:
            long_tail_rate = long_tail_rate / float(num_batch)
            return long_tail_rate, model_results
        return model_results#, attack_model_results
