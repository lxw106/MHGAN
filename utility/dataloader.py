...
class Data(object):
    ...
    def LightGCN_norm_matrix(self):
        ...
        print("\t LightGCN matrix constructed.")
    ...
    def sparse_adjacency_matrix(self):
        if self.bipartite_graph is None:
            try:
                pre_adjacency = sp.load_npz(self.path + '/s_pre_adj_mat.npz')
                ...
                adjacency_matrix[self.num_users:, :self.num_users] = R.T
                adjacency_matrix = adjacency_matrix.tocsr()
                sp.save_npz(self.path + '/s_pre_adj_mat.npz', adjacency_matrix)
